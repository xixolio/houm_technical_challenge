...
def add_hourly_temperatures_to_visits(*args):
    ...
    df = pd.DataFrame(data, columns=['scheduled_id', 'property_id', 'date', 'latitude', 'longitude', 'temp'])
    df['date'] = pd.to_datetime(df['date'])
    return df
...
class TestGetAverageTemperatureInVisitsByUser(object):
    def test_on_multiple_visits_multiple_properties(self, mocker,):
        ...
        mocker.patch('src.properties_info.properties_info.PropertiesInfo.load_users_data',
                     return_value=pd.DataFrame([], columns=['user_id', 'property_id']))
        mocker.patch('src.properties_info.properties_info.PropertiesInfo.load_properties_data',
        ...
        mocker.patch('src.properties_info.properties_info.PropertiesInfo.load_visits_data',
